...
if %dns_is_reachable%==1  echo: Internet is working, but dns is not set, so websites not opening
exit /b
:ip_is_not_set
...
if "%dns_server%"=="" (if %internet_gateway_is_reachable%==1 echo: Internet is working despite no gateway) 
if /i "%dns_server%"=="none" (if %dns_is_reachable%==1  echo: Able to ping dns server despite no gateway)
if "%dns_server%" NEQ "" if /i "%dns_server%" NEQ "none" if %dns_is_reachable%==1  echo: Able to ping dns server despite no gateway
exit /b
:not_connected
